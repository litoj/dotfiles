		...
		dap.configurations.c = {
			{
				...
				cwd = '${workspaceFolder}',
				program = function()
					local name = vim.api.nvim_buf_get_name(0):sub(#vim.fn.getcwd() + 2) -- relative path
					local seek = {
						'main.out',
						name:gsub('[^/]+$', 'main.out'),
						name:gsub('%.c$', '.out'),
...
map({ 'n', 'i' }, '<A-T>', "<C-s><Cmd>!cd '%:h' && make test<CR>", { remap = true })
map({ 'n', 'i' }, '<A-M>', "<Cmd>w|!cd '%:h' && make all<CR>")
